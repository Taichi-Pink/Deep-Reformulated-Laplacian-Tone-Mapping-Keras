...
def generate_train_data_from_file(args):
    ...
    for i in range(len(file_list)):
        ...
        for i in range(num):
            ldr_label = label_rand_patches[i]
            ldr_label = np.expand_dims(ldr_label, axis=2)
            array_label.append(np.array(ldr_label))

            img_rand_patches[i] = lpyr_gen(img_rand_patches[i], args["level"])
            label_rand_patches[i] = lpyr_gen(label_rand_patches[i], args["level"])
            ...
            for j in range(0,2):
                img_rand_patches[i][j] = np.expand_dims(img_rand_patches[i][j], axis=2)
                label_rand_patches[i][j] = np.expand_dims(label_rand_patches[i][j], axis=2)
